...
function Blouse() {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:5001/api/products')
            ...
            .then(data => {
                const blouseIds = [30, 37, 39, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107];
                const filteredBlouses = data.filter(item => blouseIds.includes(item.id));
                setProducts(filteredBlouses);
            })
            ...
            <Grid container spacing={3} justifyContent="center">
                {products.map((product, index) => (
                    <Grid item key={product.id || index} xs={6} sm={4} md={3}>
                        ...
                        <Card
                            onClick={() => handleGoToDetail(null, product)}
                            elevation={0}
                            sx={{
                                borderRadius: 4,
                                textAlign: "left",
